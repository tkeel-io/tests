 ...
 */
it("login", (done) => {
  console.log("login:",`/apis/security/v1/oauth/token?grant_type=password&username=${device.tenantInfo.tenant_id}-tanli&password=123456`)
  request
    .get(`/apis/security/v1/oauth/token?grant_type=password&username=${device.tenantInfo.tenant_id}-tanli&password=123456`)
    .expect(200)
    .then((res: any) => {
      console.log("login return ")
      let result = JSON.parse(res.text).data;
